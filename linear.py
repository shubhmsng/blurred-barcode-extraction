...
def linear(image):
    ...
    #################################################
    # step 6: perform dilation on thresholded image #
    #################################################

    dilate = cv2.dilate(threshold, None, iterations=6)

    ####################################################
    # step 7: perform erode operation on dilated image #
    ####################################################

    closed = cv2.erode(dilate, None, iterations=15)

    ##############################################
    # step 8: find contours on the image         #
    ##############################################

    contours, hierarchy = cv2.findContours(closed.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    ###################################################
    # step 9: sort all contours on the basis of area #
    ###################################################

    c = sorted(contours, key=cv2.contourArea, reverse=True)[0]

    ###########################################################################
    # step 10: find the barcode contour and get the pixel location of contour #
    ###########################################################################
    ...
    #####################################################
    # step 11: crop the barcode from the original image #
    #####################################################

    cr_img = image[y:y+h+50, x:x+w+50]

    #####################################################
    # step 12: find the width and height of the barcode #
    #####################################################
    ...
    #######################################
    # step 16: create a reader            #
    #######################################

    scanner = zbar.ImageScanner()

    #######################################
    # step 17: configure the reader       #
    #######################################

    scanner.parse_config('enable')

    #######################################
    # step 18: obtain image data          #
    #######################################
    ...
    #######################################
    # step 19: wrap image data            #
    #######################################

    image = zbar.Image(width, height, 'Y800', raw)

    ########################################
    # step 20: scan the image for barcode  #
    ########################################

    scanner.scan(image)

    #######################################
    # step 21: extract results            #
    #######################################
    result = ""
    for symbol in image:
